...
if (typeof LandscapeShellConstants === "undefined" || isNull(LandscapeShellConstants)) {
    var LandscapeShellConstants = {
        ...
        GripLayerName: "ARCH_HOSTED_GRIPS",
        BuildTag: "wall-v25"
    };
}
...
function LandscapeShellPanel(parentWidget, appWin) {
    ...
    var file = new QFile(LandscapeShellPanel.includeBasePath + "/landscape-shell-prototype-v17.ui");
    ...
    this.hostedRuntimeStates = {};
    this.hostedRuntimePendingMoves = {};
    this.hostedRuntimeRepairInProgress = false;
    this.hostedGripPendingMove = undefined;
    ...
    this.startHostedIntegrityMonitor();
    this.startHostedGripMonitor();
}
...
LandscapeShellPanel.prototype.updateHostedSelectionUi = function(info) {
    ...
    this.refreshHostedGripOverlay();
};

LandscapeShellPanel.prototype.clearHostedGripOverlay = function() {
    ...
    for (var i=0; i<ids.length; i++) {
        ...
        if (isNull(entity) || !isHostedGripEntity(entity)) {
            continue;
        }
...
LandscapeShellPanel.prototype.getHostedGripDefinitions = function(info) {
    if (isNull(info)) {
        return [];
    }
    ...
    var candidate = getHostedWallCandidateFromGroup(document, info.groupId);
    if (isNull(candidate)) {
        return [];
    }
    ...
    var descriptorResult = buildHostedOpeningDescriptors(
        candidate.axisStart.getDistanceTo(candidate.axisEnd),
        corners,
        [{
            objectId: info.objectId,
            centerT: info.centerT,
            openingWidth: info.openingWidth,
            libraryItemId: info.libraryItemId,
            libraryItem: cloneLibraryItemWithOverrides(
                cloneLibraryItemWithWidth(info.libraryItem, info.openingWidth),
                { hingeSide: info.libraryItem.hingeSide || "" }
            ),
            faceSide: info.faceSide || "",
            hingeSide: info.libraryItem.hingeSide || ""
        }]
    );
    if (!isNull(descriptorResult.error) || isNull(descriptorResult.descriptors) || descriptorResult.descriptors.length===0) {
        return [];
    }

    var descriptor = descriptorResult.descriptors[0];
    var leftCenter = interpolateVector(descriptor.topLeft, descriptor.bottomLeft, 0.5);
    var rightCenter = interpolateVector(descriptor.topRight, descriptor.bottomRight, 0.5);
    var center = interpolateVector(leftCenter, rightCenter, 0.5);

    return [
        { role: "move", position: center },
        { role: "resize-left", position: leftCenter },
        { role: "resize-right", position: rightCenter }
    ];
};

LandscapeShellPanel.prototype.refreshHostedGripOverlay = function() {
    this.clearHostedGripOverlay();

    var info = this.currentHostedSelectionInfo;
    if (isNull(info)) {
        return;
    }
    ...
    ensureLayer(document, di, LandscapeShellConstants.GripLayerName, "darkgreen");
    var gripLayer = document.queryLayer(LandscapeShellConstants.GripLayerName);
    var gripLayerId = isNull(gripLayer) ? undefined : gripLayer.getId();
    var grips = this.getHostedGripDefinitions(info);
    if (grips.length===0) {
        return;
    }
    ...
    for (var i=0; i<grips.length; i++) {
        var grip = grips[i];
        var point = new RPointEntity(document, new RPointData(grip.position));
        if (!isNull(gripLayerId)) {
            point.setLayerId(gripLayerId);
        }
        point.setColor(new RColor(grip.role==="move" ? "darkgreen" : "blue"));
        setAppCustomProperty(point, "hostObjectId", String(info.objectId));
        setAppCustomProperty(point, "hostWallId", String(info.groupId));
        setAppCustomProperty(point, "libraryItemId", String(info.libraryItemId));
        setAppCustomProperty(point, "openingWidth", String(info.openingWidth));
        setAppCustomProperty(point, "hostDistanceAlongWall", String(info.centerT));
        setAppCustomProperty(point, "hostFaceSide", String(info.faceSide || ""));
        setAppCustomProperty(point, "hostHingeSide", String(info.libraryItem.hingeSide || ""));
        setAppCustomProperty(point, "hostGripRole", grip.role);
        addOp.addObject(point, false);
    }
...
LandscapeShellPanel.prototype.getSelectedHostedEntityInfo = function() {
    ...
    for (var i=0; i<ids.length; i++) {
        ...
        var info = getHostedSelectionInfoFromEntity(entity, ids[i]);
        if (!isNull(info)) {
            return info;
        }
    }
...
LandscapeShellPanel.prototype.expandHostedSelectionToWholeObject = function() {
    var di = EAction.getDocumentInterface();
    if (isNull(di)) {
        return false;
    }
    ...
    if (isNull(selectedIds) || selectedIds.length===0) {
        return false;
    }
    ...
    for (var i=0; i<selectedIds.length; i++) {
        var entity = doc.queryEntity(selectedIds[i]);
        if (isNull(entity)) {
            continue;
        }

        if (isHostedGripEntity(entity)) {
            return false;
        }

        var selectedObjectId = String(getAppCustomProperty(entity, "hostObjectId", ""));
        if (selectedObjectId==="") {
            continue;
        }
        ...
        else if (objectId!==selectedObjectId) {
            return false;
        }
    ...
    for (var a=0; a<allIds.length; a++) {
        var allEntity = doc.queryEntity(allIds[a]);
        if (isNull(allEntity)) {
            continue;
        }

        if (String(getAppCustomProperty(allEntity, "hostObjectId", ""))===objectId) {
            if (isHostedGripEntity(allEntity)) {
                continue;
            }
            hostedIds.push(allIds[a]);
        }
    }
    ...
    if (hostedCount===hostedIds.length && selectedIds.length===hostedIds.length) {
        return false;
    }
    ...
    var selectedWithApi = false;
    try {
        if (typeof di.selectEntities === "function") {
            di.selectEntities(hostedIds);
            selectedWithApi = true;
            changed = true;
        }
    }
    catch (e2) {
        try {
            di.selectEntities(hostedIds, false);
            selectedWithApi = true;
            changed = true;
        }
        catch (e3) {
    ...
    if (!selectedWithApi) {
        for (var h=0; h<hostedIds.length; h++) {
            try {
                if (typeof di.selectEntity === "function") {
                    di.selectEntity(hostedIds[h]);
                    changed = true;
                }
                else {
                    var hostedEntity = doc.queryEntity(hostedIds[h]);
                    if (!isNull(hostedEntity)) {
                        hostedEntity.setSelected(true);
                        changed = true;
                    }
                }
            }
                ...
                try {
                    var hostedEntityFallback = doc.queryEntity(hostedIds[h]);
                    if (!isNull(hostedEntityFallback)) {
                        hostedEntityFallback.setSelected(true);
                        changed = true;
                    }
                }
                catch (e5) {
    ...
    if (changed) {
        forceDocumentRedraw(di);
        logLine("[OK] expanded hosted selection to whole object: " + objectId + ", entities=" + hostedIds.length);
        return true;
    }
...
LandscapeShellPanel.prototype.refreshHostedSelectionFromCurrentDocument = function(forceStatus) {
    var selectedGrip = this.getSelectedHostedGripEntity();
    if (!isNull(selectedGrip)) {
        if (isNull(this.currentHostedSelectionInfo)) {
            var gripInfo = getHostedSelectionInfoFromEntity(selectedGrip, -1);
            if (!isNull(gripInfo)) {
                this.currentHostedSelectionInfo = gripInfo;
                this.currentHostedSelectionKey = String(gripInfo.objectId);
            }
        }
        return;
    }

    this.expandHostedSelectionToWholeObject();
...
LandscapeShellPanel.prototype.scheduleHostedSelectionRefresh = function(delayMs) {
    var panel = this;
    var timer = new QTimer();
    timer.interval = Math.max(0, delayMs || 0);
    timer.singleShot = true;
    retainUiTimer(timer);
    timer.timeout.connect(function() {
        try {
            ...
            logLine("[ERR] hosted selection refresh failed: " + e);
        }
    });
    timer.start();
};
...
LandscapeShellPanel.prototype.startHostedSelectionMonitor = function() {
    ...
    try {
        var di = EAction.getDocumentInterface();
        if (!isNull(di) && !isNull(di.selectionChanged) && typeof di.selectionChanged.connect === "function") {
            di.selectionChanged.connect(function() {
                panel.refreshHostedSelectionFromCurrentDocument(false);
                panel.scheduleHostedSelectionRefresh(40);
            });
            logLine("[OK] hosted selection signal connected");
        }
        else if (!isNull(di) && typeof di.selectionChangedEmitter === "function") {
            var selectionEmitter = di.selectionChangedEmitter();
            if (!isNull(selectionEmitter) && typeof selectionEmitter.connect === "function") {
                selectionEmitter.connect(function() {
                    panel.refreshHostedSelectionFromCurrentDocument(false);
                    panel.scheduleHostedSelectionRefresh(40);
                });
                logLine("[OK] hosted selection emitter connected");
            }
        }
    }
    catch (e2) {
        logLine("[ERR] hosted selection signal hookup failed: " + e2);
    }
};

LandscapeShellPanel.prototype.getHostedEntityIdsByObject = function(document) {
    var idsByObject = {};
    if (isNull(document)) {
        return idsByObject;
    }

    var allIds = document.queryAllEntities();
    for (var i=0; i<allIds.length; i++) {
        var entity = document.queryEntity(allIds[i]);
        if (isNull(entity)) {
            continue;
        }
        ...
        if (!idsByObject.hasOwnProperty(objectId)) {
            idsByObject[objectId] = [];
        }
        idsByObject[objectId].push(allIds[i]);
    }

    return idsByObject;
};

LandscapeShellPanel.prototype.getSingleDoorOriginalHingePoint = function(document, state) {
    if (isNull(document) || isNull(state) || isNull(state.libraryItem) || !isSingleSwingDoorItem(state.libraryItem)) {
        return undefined;
    }

    var candidate = getHostedWallCandidateFromGroup(document, state.groupId);
    if (isNull(candidate)) {
        return undefined;
    }
    ...
    var descriptorResult = buildHostedOpeningDescriptors(
        ...
        [{
            ...
            centerT: state.centerT,
            openingWidth: state.openingWidth,
            libraryItemId: state.libraryItemId,
            libraryItem: cloneLibraryItemWithOverrides(
                cloneLibraryItemWithWidth(state.libraryItem, state.openingWidth),
                {
                    hingeSide: state.libraryItem.hingeSide || ""
                }
            ),
    ...
    var descriptor = descriptorResult.descriptors[0];
    var leftJambCenter = interpolateVector(descriptor.topLeft, descriptor.bottomLeft, 0.5);
    var rightJambCenter = interpolateVector(descriptor.topRight, descriptor.bottomRight, 0.5);
    return state.libraryItem.hingeSide==="right" ? rightJambCenter : leftJambCenter;
};

LandscapeShellPanel.prototype.getHostedObjectLiveCenterPoint = function(document, state) {
    if (isNull(document) || isNull(state)) {
        return undefined;
    }

    var entities = getHostedEntitiesForObject(document, state.objectId);
    ...
    for (var i=0; i<entities.length; i++) {
        ...
        var entityPosition = getEntityPositionVector(entity);
        if (!isNull(entityPosition)) {
            positionedPoints.push(entityPosition);
        }

        if (isLineEntity(entity)) {
            var pair = getLineEndpointPair(entity);
            if (isNull(pair)) {
                continue;
            }
            lineMidpoints.push(midpointVector(pair.start, pair.end));
            lineEndpoints.push(pair);
            continue;
        }

        var arcCenter = getArcCenterVector(entity);
        if (!isNull(arcCenter)) {
            arcCenters.push(arcCenter);
        }
    }

    var sourceCandidate = getHostedWallCandidateFromGroup(document, state.groupId);
    if (!isNull(sourceCandidate)) {
        var sourceCenterPoint = interpolateVector(sourceCandidate.axisStart, sourceCandidate.axisEnd, state.centerT);

        if (isSingleSwingDoorItem(state.libraryItem)) {
            var originalHinge = this.getSingleDoorOriginalHingePoint(document, state);
            var currentHinge = undefined;

            if (arcCenters.length>0) {
                currentHinge = arcCenters[0];
            }
            else if (lineEndpoints.length>0) {
                currentHinge = lineEndpoints[0].start;
            }
            else if (positionedPoints.length>0) {
                currentHinge = positionedPoints[0];
            }

            if (!isNull(originalHinge) && !isNull(currentHinge)) {
                return new RVector(
                    sourceCenterPoint.x + (currentHinge.x - originalHinge.x),
                    sourceCenterPoint.y + (currentHinge.y - originalHinge.y)
                );
            }
        }
    }

    if (lineMidpoints.length>0) {
        return averageVectors(lineMidpoints);
    }

    if (arcCenters.length>0) {
        return averageVectors(arcCenters);
    }

    if (positionedPoints.length>0) {
        return averageVectors(positionedPoints);
    }

    return undefined;
};

LandscapeShellPanel.prototype.collectHostedRuntimeStates = function(document) {
    var states = {};
    var idsByObject = this.getHostedEntityIdsByObject(document);

    for (var objectId in idsByObject) {
        if (!idsByObject.hasOwnProperty(objectId)) {
            continue;
        }

        var entityIds = idsByObject[objectId];
        if (isNull(entityIds) || entityIds.length===0) {
            continue;
        }

        var entity = document.queryEntity(entityIds[0]);
        var info = getHostedSelectionInfoFromEntity(entity, entityIds[0]);
        if (isNull(info) || isNull(info.libraryItem)) {
            continue;
        }
        ...
        var liveCenterPoint = this.getHostedObjectLiveCenterPoint(document, info);
        states[objectId] = {
            objectId: objectId,
            entityIds: entityIds,
            entityCount: entityIds.length,
            expectedEntityCount: getExpectedHostedEntityCount(info.libraryItem),
            groupId: info.groupId,
            libraryItemId: info.libraryItemId,
            libraryItem: info.libraryItem,
            openingWidth: info.openingWidth,
            centerT: info.centerT,
            faceSide: info.faceSide,
            liveCenterPoint: liveCenterPoint
        };
    }
...
LandscapeShellPanel.prototype.applyHostedRepositionBuildResult = function(di, buildResult) {
...
LandscapeShellPanel.prototype.removeHostedItemFromRuntimeState = function(document, di, state, statusText) {
    if (isNull(state)) {
        return false;
    }

    var helperAction = new HostedLibraryPlacementAction(undefined, this, state.libraryItem);
    var buildResult = helperAction.buildHostedRemovalFromState(document, di, state);
    if (!isNull(buildResult.error)) {
        logLine("[ERR] hosted runtime removal repair: " + buildResult.error);
        return false;
    }
    ...
    if (this.currentHostedSelectionKey===String(state.objectId)) {
        this.updateHostedSelectionUi(undefined);
    }

    if (!isNull(statusText) && statusText!=="") {
        this.setScaleReferenceStatus(statusText);
    }
    logLine("[OK] hosted runtime removal repair: object=" + state.objectId);
    return true;
};

LandscapeShellPanel.prototype.repositionHostedItemFromRuntimeState = function(document, di, state) {
    if (isNull(state) || isNull(state.liveCenterPoint)) {
        return false;
    }
    ...
    var projection = projectPointToSegmentInfo(state.liveCenterPoint, sourceCandidate.axisStart, sourceCandidate.axisEnd);
    if (isNull(projection)) {
        return false;
    }

    var targetCandidate = {
        sourceEntity: sourceCandidate.sourceEntity,
        groupId: sourceCandidate.groupId,
        axisStart: sourceCandidate.axisStart,
        axisEnd: sourceCandidate.axisEnd,
        thickness: sourceCandidate.thickness,
        phase: sourceCandidate.phase,
        centerPoint: projection.point,
        centerT: projection.t
    };

    var helperAction = new HostedLibraryPlacementAction(undefined, this, state.libraryItem);
    var buildResult = helperAction.buildHostedRepositionOperations(document, di, state, targetCandidate);

    if (!isNull(buildResult.error)) {
        var restoreCandidate = {
            sourceEntity: sourceCandidate.sourceEntity,
            groupId: sourceCandidate.groupId,
            axisStart: sourceCandidate.axisStart,
            axisEnd: sourceCandidate.axisEnd,
            thickness: sourceCandidate.thickness,
            phase: sourceCandidate.phase,
            centerPoint: interpolateVector(sourceCandidate.axisStart, sourceCandidate.axisEnd, state.centerT),
            centerT: state.centerT
        };
        buildResult = helperAction.buildHostedRepositionOperations(document, di, state, restoreCandidate);
        if (!isNull(buildResult.error)) {
            logLine("[ERR] hosted runtime move repair failed: " + buildResult.error);
            return false;
        }
    }

    this.applyHostedRepositionBuildResult(di, buildResult);
    forceDocumentRedraw(di);

    var updatedInfo = {
        entityId: state.entityIds[0],
        objectId: state.objectId,
        groupId: sourceCandidate.groupId,
        libraryItemId: state.libraryItemId,
    ...
    this.setScaleReferenceStatus(updatedInfo.libraryItem.label + " moved along wall.");
    logLine("[OK] hosted runtime move repair: object=" + state.objectId + ", centerT=" + updatedInfo.centerT.toFixed(4));
    return true;
};

LandscapeShellPanel.prototype.monitorHostedRuntimeChanges = function() {
    if (this.hostedRuntimeRepairInProgress===true) {
        return;
    }
    ...
    var currentStates = this.collectHostedRuntimeStates(document);
    var previousStates = this.hostedRuntimeStates || {};

    for (var previousId in previousStates) {
        if (!previousStates.hasOwnProperty(previousId)) {
            continue;
        }

        if (!currentStates.hasOwnProperty(previousId)) {
            this.hostedRuntimeRepairInProgress = true;
            try {
                if (this.removeHostedItemFromRuntimeState(document, di, previousStates[previousId], previousStates[previousId].libraryItem.label + " deleted and wall healed.")) {
                    this.hostedRuntimeStates = this.collectHostedRuntimeStates(document);
                    delete this.hostedRuntimePendingMoves[previousId];
                    return;
                }
            }
            finally {
    ...
    for (var objectId in currentStates) {
        if (!currentStates.hasOwnProperty(objectId)) {
            continue;
        }

        var state = currentStates[objectId];
        var expectedCount = state.expectedEntityCount;
        if (expectedCount>0 && state.entityCount < expectedCount) {
            this.hostedRuntimeRepairInProgress = true;
            try {
                if (this.removeHostedItemFromRuntimeState(document, di, state, state.libraryItem.label + " deleted and wall healed.")) {
                    this.hostedRuntimeStates = this.collectHostedRuntimeStates(document);
                    delete this.hostedRuntimePendingMoves[objectId];
                    return;
                }
            }
            finally {
        ...
        if (isNull(state.liveCenterPoint)) {
            delete this.hostedRuntimePendingMoves[objectId];
            continue;
        }
        ...
        var sourceCenterPoint = interpolateVector(sourceCandidate.axisStart, sourceCandidate.axisEnd, state.centerT);
        var moveDistance = sourceCenterPoint.getDistanceTo(state.liveCenterPoint);
        if (moveDistance <= 0.75) {
            delete this.hostedRuntimePendingMoves[objectId];
            continue;
        }

        var pending = this.hostedRuntimePendingMoves[objectId];
        if (isNull(pending)) {
            pending = {
                x: state.liveCenterPoint.x,
                y: state.liveCenterPoint.y,
                stableTicks: 1
            };
        }
        else {
            var dx = Math.abs(pending.x - state.liveCenterPoint.x);
            var dy = Math.abs(pending.y - state.liveCenterPoint.y);
            if (dx <= 0.05 && dy <= 0.05) {
                pending.stableTicks++;
            }
            else {
                pending.x = state.liveCenterPoint.x;
                pending.y = state.liveCenterPoint.y;
                pending.stableTicks = 1;
            }
        }

        this.hostedRuntimePendingMoves[objectId] = pending;
        if (pending.stableTicks < 4) {
            continue;
        }

        this.hostedRuntimeRepairInProgress = true;
        try {
            if (this.repositionHostedItemFromRuntimeState(document, di, state)) {
                this.hostedRuntimeStates = this.collectHostedRuntimeStates(document);
                delete this.hostedRuntimePendingMoves[objectId];
                return;
            }
    ...
    this.hostedRuntimeStates = currentStates;
};

LandscapeShellPanel.prototype.getSelectedHostedGripEntities = function() {
    var grips = [];
    var di = EAction.getDocumentInterface();
    if (isNull(di)) {
        return grips;
...
LandscapeShellPanel.prototype.getSelectedHostedGripEntity = function() {
    var grips = this.getSelectedHostedGripEntities();
    return grips.length>0 ? grips[0] : undefined;
};

LandscapeShellPanel.prototype.applyHostedGripAction = function(info, role, actualPosition) {
    if (isNull(info) || isNull(actualPosition)) {
        return false;
    }
    ...
    var projection = projectPointToSegmentInfo(actualPosition, candidate.axisStart, candidate.axisEnd);
    if (isNull(projection)) {
        return false;
    }

    var helperAction = new HostedLibraryPlacementAction(undefined, this, info.libraryItem);
    ...
    if (role==="move") {
        buildResult = helperAction.buildHostedRepositionOperations(document, di, nextInfo, {
        ...
        var halfT = (info.openingWidth / wallLength) / 2.0;
        var fixedT = role==="resize-left" ? (info.centerT + halfT) : (info.centerT - halfT);
        var movedT = projection.t;
        var minGapT = 0.05;
        if (role==="resize-left") {
            movedT = Math.min(movedT, fixedT - (0.5 / wallLength));
            if (movedT<=minGapT) {
                movedT = minGapT;
            }
        }
        else {
            movedT = Math.max(movedT, fixedT + (0.5 / wallLength));
            if (movedT>=1.0-minGapT) {
                movedT = 1.0-minGapT;
            }
        }

        var newCenterT = (fixedT + movedT) / 2.0;
        var newWidth = Math.abs(fixedT - movedT) * wallLength;
        nextInfo.openingWidth = newWidth;
        nextInfo.centerT = newCenterT;
        nextInfo.libraryItem = cloneLibraryItemWithOverrides(
            cloneLibraryItemWithWidth(info.libraryItem, newWidth),
            { hingeSide: info.libraryItem.hingeSide || "" }
        );
        buildResult = helperAction.buildHostedRepositionOperations(document, di, nextInfo, {
            sourceEntity: candidate.sourceEntity,
            ...
            thickness: candidate.thickness,
            phase: candidate.phase,
            centerPoint: interpolateVector(candidate.axisStart, candidate.axisEnd, newCenterT),
            centerT: newCenterT
        });
    }
    ...
    if (!isNull(buildResult.error)) {
        ...
        logLine("[ERR] hosted grip action: " + buildResult.error);
        return false;
    }
    ...
    this.updateHostedSelectionUi(updatedInfo);
    this.setScaleReferenceStatus(role==="move" ?
        (updatedInfo.libraryItem.label + " moved by grip.") :
        (updatedInfo.libraryItem.label + " resized by grip to " + updatedInfo.openingWidth.toFixed(2) + "."));
    return true;
};

LandscapeShellPanel.prototype.monitorHostedGripChanges = function() {
    var gripEntities = this.getSelectedHostedGripEntities();
    if (isNull(gripEntities) || gripEntities.length===0) {
        this.hostedGripPendingMove = undefined;
        return;
    }

    var info = this.currentHostedSelectionInfo;
    if (isNull(info) || String(info.objectId)==="") {
        info = getHostedSelectionInfoFromEntity(gripEntities[0], -1);
    }
    if (isNull(info)) {
        this.hostedGripPendingMove = undefined;
        return;
    ...
    var defs = this.getHostedGripDefinitions(info);
    if (isNull(defs) || defs.length===0) {
        this.hostedGripPendingMove = undefined;
        return;
    }
    ...
    for (var d=0; d<defs.length; d++) {
        defsByRole[defs[d].role] = defs[d];
    }

    var chosenGrip = undefined;
    var chosenRole = "";
    var chosenPosition = undefined;
    var chosenDistance = -1;

    for (var i=0; i<gripEntities.length; i++) {
        var gripEntity = gripEntities[i];
        var objectId = String(getAppCustomProperty(gripEntity, "hostObjectId", ""));
        if (objectId!==String(info.objectId)) {
            continue;
        }

        var role = getHostedGripRole(gripEntity);
        var actualPosition = getHostedGripPosition(gripEntity);
        var expected = defsByRole[role];
        if (role==="" || isNull(actualPosition) || isNull(expected) || isNull(expected.position)) {
            continue;
        }

        var distance = expected.position.getDistanceTo(actualPosition);
        if (distance > chosenDistance) {
            chosenGrip = gripEntity;
            chosenRole = role;
    ...
    if (isNull(chosenGrip) || chosenRole==="" || isNull(chosenPosition)) {
        this.hostedGripPendingMove = undefined;
        return;
    }

    logLine("[OK] hosted grip monitor: role=" + chosenRole + ", object=" + info.objectId + ", selectedGrips=" + gripEntities.length + ", distance=" + chosenDistance.toFixed(3));
    if (chosenDistance<=0.35) {
        this.hostedGripPendingMove = undefined;
        return;
    }

    var pending = this.hostedGripPendingMove;
    if (isNull(pending) || pending.objectId!==info.objectId || pending.role!==chosenRole) {
        pending = {
            objectId: info.objectId,
            role: chosenRole,
            x: chosenPosition.x,
        ...
        var dx = Math.abs(pending.x - chosenPosition.x);
        var dy = Math.abs(pending.y - chosenPosition.y);
        if (dx<=0.05 && dy<=0.05) {
            pending.stableTicks++;
        }
        else {
            pending.x = chosenPosition.x;
            pending.y = chosenPosition.y;
            pending.stableTicks = 1;
        }
    }
    this.hostedGripPendingMove = pending;
    if (pending.stableTicks < 2) {
        return;
    }

    this.hostedGripPendingMove = undefined;
    this.applyHostedGripAction(info, chosenRole, chosenPosition);
};

LandscapeShellPanel.prototype.startHostedGripMonitor = function() {
    var panel = this;
    var timer = new QTimer();
    timer.interval = 180;
    timer.singleShot = false;
    ...
    timer.timeout.connect(function() {
        try {
            panel.monitorHostedGripChanges();
        }
        catch (e) {
            logLine("[ERR] hosted grip monitor failed: " + e);
        }
    });
    timer.start();
};

LandscapeShellPanel.prototype.startHostedIntegrityMonitor = function() {
    var panel = this;
    var timer = new QTimer();
    timer.interval = 220;
    timer.singleShot = false;
    ...
    timer.timeout.connect(function() {
        try {
            panel.monitorHostedRuntimeChanges();
        }
        catch (e) {
            logLine("[ERR] hosted runtime monitor failed: " + e);
        }
    });
    timer.start();
};
...
function formatHostedBlockToken(value) {
    if (isNull(value) || isNaN(value)) {
        return "0";
    }

    var normalized = String(Math.round(value * 1000));
...
function getHostedSingleDoorBlockName(openingWidth, deltaRadians) {
    return "HostedSingleDoor_" + formatHostedBlockToken(openingWidth) + "_" + formatHostedBlockToken(deltaRadians);
}

function ensureHostedSingleDoorBlock(document, di, openingWidth, deltaRadians) {
    if (isNull(document) || isNull(di)) {
        return undefined;
    }

    var blockName = getHostedSingleDoorBlockName(openingWidth, deltaRadians);
    var existingBlockId = -1;
    try {
        existingBlockId = document.getBlockId(blockName);
    }
    catch (e) {
    }

    if (!isNaN(existingBlockId) && existingBlockId >= 0) {
        return blockName;
    }

    var block = new RBlock(document, blockName, new RVector(0, 0));
    di.applyOperation(new RAddObjectOperation(block));

    try {
        di.setCurrentBlock(blockName);

        var leafEnd = new RVector(
            openingWidth * Math.cos(deltaRadians),
            openingWidth * Math.sin(deltaRadians)
        );
        var doorLeaf = new RLineEntity(document, new RLineData(new RVector(0, 0), leafEnd));
        var doorArc = new RArcEntity(
            document,
            new RArcData(new RVector(0, 0), openingWidth, 0.0, deltaRadians, deltaRadians < 0)
        );

        doorLeaf.setColor(new RColor("black"));
        doorArc.setColor(new RColor("black"));

        var addOp = new RAddObjectsOperation();
        addOp.addObject(doorLeaf, false);
        addOp.addObject(doorArc, false);
        di.applyOperation(addOp);
    }
    finally {
        try {
...
function getExpectedHostedEntityCount(item) {
    if (isNull(item)) {
        return 0;
    }
    ...
    if (item.category==="Doors") {
        if (item.swingType==="single") {
            return 1;
        }
        if (item.swingType==="double") {
            return 4;
        }
        return 2;
    }
...
function averageVectors(vectors) {
    if (isNull(vectors) || vectors.length===0) {
        return undefined;
    }
    ...
    return new RVector(x / vectors.length, y / vectors.length);
}

function midpointVector(startPoint, endPoint) {
    return new RVector(
...
function getEntityPositionVector(entity) {
    if (isNull(entity)) {
        return undefined;
    }
...
function getHostedGripRole(entity) {
    if (isNull(entity)) {
        return "";
    }
    return String(getAppCustomProperty(entity, "hostGripRole", ""));
}

function isHostedGripEntity(entity) {
    return getHostedGripRole(entity)!=="";
}

function getHostedGripPosition(entity) {
    if (isNull(entity)) {
        return undefined;
    }

    var pos = getEntityPositionVector(entity);
    if (!isNull(pos)) {
        return pos;
    }
    ...
    try {
        ...
        if (!isNull(data)) {
            ...
            if (typeof data.getCenterPoint === "function") {
                return data.getCenterPoint();
            }
            if (typeof data.getCenter === "function") {
...
function getArcCenterVector(entity) {
    if (isNull(entity)) {
        return undefined;
    }
...
function getLineEndpointPair(entity) {
    if (isNull(entity) || !isLineEntity(entity)) {
        return undefined;
    }
    ...
    try {
        ...
        return {
            start: data.getStartPoint(),
            end: data.getEndPoint()
        };
    }
    catch (e) {
    }
...
function getHostedSelectionInfoFromEntity(entity, entityId) {
    if (isNull(entity)) {
        return undefined;
    }
...
HostedLibraryPlacementAction.prototype.appendSymbolEntitiesForDescriptor = function(document, di, candidate, corners, descriptor, symbolAddOp) {
    ...
    if (libraryItem.category==="Doors") {
        ...
        if (libraryItem.swingType==="double") {
            ...
            var blockName = ensureHostedSingleDoorBlock(document, di, openingWidth, arcDelta);
            if (isNull(blockName) || blockName==="") {
                return "Could not create the hosted door symbol.";
            }

            var blockId = document.getBlockId(blockName);
            if (isNaN(blockId) || blockId < 0) {
                return "Could not resolve the hosted door symbol.";
            }

            var blockRef = new RBlockReferenceEntity(
                document,
                new RBlockReferenceData(blockId, hingePoint, new RVector(1, 1), closedAngle)
            );
            if (!isNull(symbolLayerId)) {
                blockRef.setLayerId(symbolLayerId);
            }
            blockRef.setColor(new RColor("black"));
            this.addHostedMetadata(blockRef, candidate, hostedObject, rotationDegrees);
            symbolAddOp.addObject(blockRef, false);
        }
    }
    else {
...
HostedLibraryPlacementAction.prototype.buildHostedRemovalOperations = function(document, di, selectedInfo) {
    return this.buildHostedRemovalFromState(document, di, selectedInfo);
};

HostedLibraryPlacementAction.prototype.buildHostedRemovalFromState = function(document, di, selectedInfo) {
    var candidate = getHostedWallCandidateFromGroup(document, selectedInfo.groupId);
    if (isNull(candidate)) {
        return {
            error: "Could not resolve the wall for the selected hosted item."
...
HostedLibraryPlacementAction.prototype.buildHostedRepositionOperations = function(document, di, selectedInfo, targetCandidate) {
    ...
    movingObject.faceSide = selectedInfo.faceSide || movingObject.faceSide || "";
...
function registerLandscapeWallGuiAction() {
    ...
    var scriptFile = LandscapeShellPanel.includeBasePath + "/LandscapeShellWallAction-v17.js";
...
function main() {
    ...
    safeCall("add landscape dock", function() {
        ...
        var scrollArea = new QScrollArea(dock);
        try {
            scrollArea.setWidgetResizable(true);
            scrollArea.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff);
            if (typeof scrollArea.setFrameShape === "function" && typeof QFrame !== "undefined" && !isNull(QFrame)) {
                scrollArea.setFrameShape(QFrame.NoFrame);
            }
        }
        catch (eScrollAreaSetup) {
        }

        try {
            panel.setMinimumHeight(0);
        }
        catch (ePanelMinHeight) {
        }
